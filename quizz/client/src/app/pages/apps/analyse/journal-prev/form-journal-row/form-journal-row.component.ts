...
import { formInputTxt } from '../journal-prev.data';
...
export class FormJournalRowComponent implements OnInit {
  ...
  constructor(private router: Router, private fyService: FyService, private formBuilder: FormBuilder, private jrService: JournalRowService, private nsService: NsService, private compteService: CompteService) {
    this.txt = formInputTxt.find((item) => item.action == 'create')
    console.log(this.txt)
  }

  ngOnInit() {
    const { data, action } = history.state
    if (history.state) {
  ...
  crudJr() {
    ...
    if (this.journalRowForm.valid) {
      ...
      this.jrService.post(jr).subscribe(() => {
        console.log('creer')
        this.router.navigate(['apps/journal'])
      })
    }
  }

  close() {
    this.router.navigate(['apps/journal-prev'])
  }
